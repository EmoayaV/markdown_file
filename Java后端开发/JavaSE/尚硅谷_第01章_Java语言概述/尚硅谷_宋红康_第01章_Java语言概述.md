...
- **Java SE(Java Standard Edition)标准版（现在基本不用，可以作为基础学）**
  - 支持面向`桌面级应用`（如Windows下的应用程序）的Java平台，即定位个人计算机的应用开发。
    - 包括用户界面接口AWT及Swing，网络功能与国际化、图像处理能力以及输入输出支持等。
  - 此版本以前称为J2SE
- **Java EE(Java Enterprise Edition)企业版（目前主要用的）**
  ...
  - 版本以前称为J2EE
- **Java ME(Java Micro Edition)小型版（现在基本不用）**
...
#### 7.5.3 JDK8配置方案2：配置JAVA_HOME+path（推荐）（掌握）
* 步骤：
...
#### 14.2.2 功能2：自动内存管理(内存分配、内存回收)（掌握）
...
>
> 内存溢出指的是输入内存的数据大于内存的容量，程序就会报错
>
> 内存泄漏在java中指我认为是垃圾的数据JVM不认为是垃圾，所以就没有回收这部分内存
>
> 内存泄漏是不容易发现的，但是内存泄露多了会导致内存溢出
