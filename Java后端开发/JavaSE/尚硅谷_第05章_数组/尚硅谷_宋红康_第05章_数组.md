...
| 区域名称   | 作用                                                      |
| ----------| ---------------------------------------------------------|
| `虚拟机栈` | 用于存储正在执行的每个Java方法的**局部变量**表等。局部变量表存放了编译期可知长度的各种**基本数据类型、对象引用**，方法执行完，自动释放。 |
...
![image-20230404200037691](D:\markdown file\截图\image-20230404200037691.png)

![image-20230404201053424](D:\markdown file\截图\image-20230404201053424.png)

只有出现new了才会在堆内存中开辟一块新空间。
...
![image-20230405164414983](D:\markdown file\截图\image-20230405164414983.png)
...
- **衡量排序算法的优劣：**
  ...
  - 常见的算法时间复杂度由小到大依次为：Ο(1)＜Ο(log~2~n)＜Ο(n)＜Ο(nlog~2~n)＜Ο(n<sup>2</sup>)＜Ο(n<sup>3</sup>)＜…＜Ο(2<sup>n</sup>)＜Ο(n!)<O(n<sup>n</sup>)
