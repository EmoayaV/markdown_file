...
```java
boolean b = true;
//如果写成if(b=false)能编译通过吗？如果能，结果是？ 输出c
if(b == false) 	 //建议：if(!b)
...
class MathRandomTest {
	public static void main(String[] args) {
		double value = Math.random(); // random()调用后，会返回一个[0.0, 1.0)范围的double型随机数
...
// System.out.println() 和 Math.random() 都是java.lang包下的类，所以不用创建对象
```
