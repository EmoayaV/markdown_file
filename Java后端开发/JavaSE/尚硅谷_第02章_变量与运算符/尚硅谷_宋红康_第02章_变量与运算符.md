...
- 定义：**被Java语言赋予了特殊含义，用做专门用途的字符串（或单词）**
  - HelloWorld案例中，出现的关键字（蓝色）有 `class`、`public `  、 `static` 、  `void`  等，这些单词已经被Java定义好了。
- 特点：全部关键字都是`小写字母`。
...
**标识符的命名规范**（建议遵守的`软性要求`，否则工作时容易被鄙视）（掌握）:

```
> 包名：多单词组成时所有字母都小写：xxxyyyzzz。
  例如：java.lang、com.atguigu.bean
  
> 类名、接口名：多单词组成时，所有单词的每个单词首字母大写：XxxYyyZzz
...
- 变量的概念：
  ...
  - Java中变量声明的格式：`数据类型 变量名 = 变量值`（= 是赋值的意思）
...
### 4.1 整数类型：byte、short、int、long（掌握）
...
- 定义long类型的变量，赋值时需要以"`l`"或"`L`"作为后缀。（掌握）

- Java程序中变量通常声明为int型，除非不足以表示较大的数，才使用long。

- Java的整型`常量默认为 int 型`。（掌握）
...
- float：`单精度`，尾数可以精确到7位有效数字。很多情况下，精度很难满足需求。（float虽然表数范围大甚至比long的范围都大，但是float类型的精度是有限的，不是在这个范围内的所有数字都可以表示）   
- double：`双精度`，精度是float的两倍。通常采用此类型。
- 定义float类型的变量，赋值时需要以"`f`"或"`F`"作为后缀。（掌握）
- Java 的浮点型`常量默认为double型`。（掌握）
...
### 5.3 基本数据类型与String的运算（掌握）

#### 5.3.1 字符串类型：String（掌握）
...
System.out.println(3+4+"Hello!");      //输出：yes 7Hello
System.out.println("Hello!"+3+4);      //输出：yes Hello34
System.out.println('a'+1+"Hello!");    //输出：yes 98Hello
System.out.println("Hello"+'a'+1);     //输出：yes Helloa1
```
...
![image-20230326144206233](D:\markdown file\截图\image-20230326144206233.png)

![image-20230326191114629](D:\markdown file\截图\image-20230326191114629.png)

<img src="D:\markdown file\截图\image-20230328162808701.png" alt="image-20230328162808701" style="zoom: 67%;" />
...
* 第一种：对于`+`两边都是数值（char也算）的话，`+`就是加法的意思
...
>if(b1 & (num1++ > 0)){ }  // 先运算后加一，就是先拿10判断是否>0，然后再把num1 加一
...
//(1)先取i的值“2”放操作数栈 (2)i再自增,i=3 (3)再把操作数栈中的"2"赋值给j,j=2

int i = 2;
int j = ++i;
System.out.println(j); // 3
//(1)i先自增,i=3 (2)再取i的值“3”放操作数栈 (3)再把操作数栈中的"3"赋值给j,j=3

int m = 2;
m = m++; //千万不要这么写，想加1直接写m++不要赋值
...
* 自增1推荐：i++;（不改变原有的数据类型）
* 自加2推荐：i += 2;（不改变原有的数据类型）-=，*=，/=，%= 是相同的道理，都推荐这么用
...
![image-20230502141059639](D:\markdown file\截图\image-20230502141059639.png)
