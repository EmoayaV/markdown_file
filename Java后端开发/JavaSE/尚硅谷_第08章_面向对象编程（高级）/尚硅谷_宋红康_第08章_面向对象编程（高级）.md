...
![image-20230419153642899](D:\markdown file\截图\image-20230419153642899.png)

![image-20230419144142633](D:\markdown file\截图\image-20230419144142633.png)

![image-20230423140445206](D:\markdown file\截图\image-20230423140445206.png)

![image-20230423140827725](D:\markdown file\截图\image-20230423140827725.png)
...
- **在类方法（就是静态方法）中不能调用非静态的方法和属性（因为非静态的属性和方法前面都默认省略了this. ，使用类调用静态方法时对象还没有创建，所以不能使用非静的属性方法，即实例变量和实例方法）**

- **静态变量，随着类的加载而加载，由于类只会加载一次，所以静态变量也只有一份。**

- **实例变量，随着对象的创建而加载，由于对象可能会创建多次，所以实例变量也会有多份。**

- **类先加载，对象在对象加载后才会加载。**

- **static修饰的方法内，不能使用this和super，因为this和super只有在对象创建后才可以使用。**

- **静态方法只能调用静态的属性**

- **在static方法内部只能访问类的static修饰的属性或方法，不能访问类的非static的结构（如果要访问需要先实例化对象）。**
...
- 先把类放在方法区，进行类的加载，一定先加载父类Object，再加载子类
  - 加载类的时候执行(静态变量放在堆空间中的静态域) ①静态变量的默认初始化 ②静态变量显式初始化 或 静态代码块中初始化(主要看哪个写在前面) ③静态方法的加载
...
当我们编写一个类时，其实就是在描述其对象的属性和行为，而并没有产生实质上的对象，只有通过new关键字才会产出对象，这时系统才会分配内存空间给对象，其方法才可以供外部调用。**我们有时候希望无论是否产生了对象或无论产生了多少对象的情况下某些特定的数据在内存空间里只有一份**。例如，所有的中国人都有个国家名称，每一个中国人都共享这个国家名称，不必在每一个中国人的实例对象中都单独分配一个用于代表国家名称的变量。

![image-20220325213629311](images/image-20220325213629311.png)



此外，在类中声明的**实例方法**，在类的外面必须要先创建对象，才能调用。但是有些方法的调用者和当前类的对象无关，这样的方法通常被声明为`类方法`，由于不需要创建对象就可以调用类方法，从而简化了方法的调用。

这里的**类变量、类方法**，只需要使用`static`修饰即可。所以也称为静态变量、静态方法。
...
- **静态代码块的使用场景一般为在初始化一个静态方法/类方法时，不能使用构造器来初始化（构造器是对于对象而言的，没有创建对象就不能调用构造器），所以使用静态代码块来代替构造器的作用。**
...
- **不能用abstract修饰私有方法、静态方法、final的方法、final的类。（主要是为了自洽）**
  - **子类必须重写抽象父类的所有抽象方法**，私有方法子类看不到，私有方法不能重写，无法重写抽象类，所以不能用abstract修饰私有方法
  - **抽象方法不能被调用**，那么调用方法由两种方式，一种是通过创建对象调用，二是通过类调用，抽象类无法创建对象，所以不能通过对象调用抽象方法，不能用abstract修饰静态方法，所以不能通过类调用抽象方法。
  - **抽象方法一定要被重写**，但是final修饰的方法无法被重写。
...
内部接口也称为[嵌套](https://so.csdn.net/so/search?q=嵌套&spm=1001.2101.3001.7020)接口，即在一个接口内部定义另一个接口。举个例子，Entry接口定义在Map接口里面

为什么一个接口中可以再定义接口呢？
就是你只实现了外部接口，那么就必须实现外部接口的所有方法（不实现所有抽象方法那么实现类就是一个抽象类）；而要用内部的接口的方法只能实现内部接口。
...
- **接口的本质是契约、标准、规范，就像我们的法律一样。制定好后大家都要遵守。而不提供具体的实现。具体实现类的集合就叫做驱动(Driver)。接口在多态可以理解为父类。**

![image-20230424162821357](D:\markdown file\截图\image-20230424162821357.png)

****

![image-20230424202956624](D:\markdown file\截图\image-20230424202956624.png)

- **接口只有属性（全局常量，必须使用public static final修饰）和方法（抽象方法，修饰为public abstract，抽象方法的返回值可以是void，也可以是int、double..，后续重写的方法返回值必须和抽象方法的返回值数据类型保持一致）**

- **接口可以继承**

- **接口和类是实现关系，接口和接口是继承关系（可以多继承，一个子类有多个父类）**
...
- **接口的多态性： 接口名 变量名 = new 实现类对象**
  ...
  **接口的多态性，编译看左边，执行看右边，f1只能调用在Bullet()类中，重写的Flyable抽象类的方法，是虚方法调用**

  **Flyable f1 = new Bullet();**

- **如果有方法形参需要传入一个接口类型的数据，比如： transformer(Interface interface){..}，则使用以下方式传入**

  - **transformer(new Camera())**

    **这里的Camera是interface接口的具体实现类，具体实现类中需要重写接口中的抽象方法。**

  - **transformer(new Interface(){重写抽象方法})**

    **这里的Interface是interface接口的匿名实现类，匿名实现类中需要重写接口中的抽象方法。**

- **接口中大量使用到了多态，比如：Interface interface = new Camera()。注意：多态只能用实现类中重写过接口抽象方法的方法，不能用实现类独有的方法。（多态只能用子类中重写过父类方法的方法，不能用子类独有的方法。）**

- 接口中如果使用了默认方法那么其子类可以继承

![image-20230424211634927](D:\markdown file\截图\image-20230424211634927.png)

![image-20230424211707605](D:\markdown file\截图\image-20230424211707605.png)
...
![image-20230426125358985](D:\markdown file\截图\image-20230426125358985.png)

![image-20230426125443371](D:\markdown file\截图\image-20230426125443371.png)

![image-20230426125557436](D:\markdown file\截图\image-20230426125557436.png)

![image-20230426132140775](D:\markdown file\截图\image-20230426132140775.png)
...
- **枚举类也是类，枚举类在类内声明对象时与普通类不同，枚举类的默认父类是Enum（Enum类的父类是Object），而普通类的默认父类是Object**
- **枚举类不能继承其他类了（因为已经继承了java.lang.Enum）**
- **枚举类的toString方法默认返回变量名称（类的toString方法默认返回 类名@地址），其中toString方法可以重写，重写后用name方法代替toString方法的作用**
- 枚举类的values方法，通过 Season[] values =  Season.values() 的方式将枚举类的所有对象放入对象数组。（其中Season是枚举类）。
- 枚举类的valuesOf方法，通过Season season = Season.valueOf(objName) 返回当前枚举类中名称为objName的枚举类对象。如果枚举类中不存在objName名称的对象，则报错。（objName是字符串类型的数据）
- 枚举类作为接口的实现类，枚举类实现接口，在枚举类中重写接口中的抽象方法。当通过不同的枚举类对象调用此方法时，执行的是同一个方法。让枚举类的每一个对象重写接口中的抽象方法。当通过不同的枚举类对象调用此方法时，执行的是不同的实现的方法。
...
![image-20230501162336608](D:\markdown file\截图\image-20230501162336608.png)

![image-20230501162425776](D:\markdown file\截图\image-20230501162425776.png)
...
- 缓存处理的原理为：如果数据在-128~127这个区间，那么在[类加载](https://so.csdn.net/so/search?q=类加载&spm=1001.2101.3001.7020)时就已经为该区间的每个数值创建了对象，并将这256个对象存放到一个名为cache的数组中。
