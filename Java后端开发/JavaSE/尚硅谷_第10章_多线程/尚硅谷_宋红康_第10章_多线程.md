...
![image-20230506204331606](D:\markdown file\截图\image-20230506204331606.png)

![image-20230506204357607](D:\markdown file\截图\image-20230506204357607.png)

![image-20230506204418313](D:\markdown file\截图\image-20230506204418313.png)

![image-20230506204550169](D:\markdown file\截图\image-20230506204550169.png)

创建线程的四种方式。

- new Thread().start()：表示调用Thread类匿名对象的start方法。
- new Thread(){}.start()：表示调用Thread类子类匿名对象的start方法。
- 看创建了多少线程，非匿名类：就看new了多少Thread的子类对象，或者new了多少Thread类的对象。匿名类：看new了多少Thread类的对象
- 对比两种方式：

  - 共同点：① 启动线程，使用的都是Thread类中定义的start() ② 创建的线程对象，都是Thread类或其子类的实例。

  - 不同点：一个是类的继承，一个是接口的实现。
  - 建议：建议使用实现Runnable接口的方式。Runnable方式的好处：① 实现的方式，避免的类的单继承的局限性② 更适合处理有共享数据的问题。③ 实现了代码和数据的分离。
...
![image-20230507142947007](D:\markdown file\截图\image-20230507142947007.png)

![image-20230507143056635](D:\markdown file\截图\image-20230507143056635.png)

![image-20230507180334915](D:\markdown file\截图\image-20230507180334915.png)
...
![image-20230508150542625](D:\markdown file\截图\image-20230508150542625.png)
...
要解决上述多线程并发访问一个资源的安全性问题:也就是解决重复票与不存在票问题，Java中提供了**同步机制**
**(synchronized)，或者叫同步锁机制**来解决。
...
**synchronized(同步监视器){}，锁就是同步监视器，同步监视器这个理必须传入一个对象，对象是哪一个类的对象没有要求，唯一的要求是这个对象必须是唯一的。**

**同步锁对象可以是任意类型**，但是必须保证竞争“同一个共享资源”的**多个线程必须使用同一个“同步锁对象**”。
...
- 死锁简单来说就是线程a要先握着锁1再握着锁2，线程b要先握着锁2再握着锁1，如果出现：线程a握着锁1还没拿到锁2时，线程b已经拿到了锁2，此时就出现了死锁，线程a等待拿锁2但是锁2已经被线程b拿了，线程b等待拿锁1但是锁1已经被线程a拿了。
...
针对条件2：可以考虑一次性申请所有所需的资源（一次性拿到所有的锁），这样就不存在等待的问题。

针对条件3：占用部分资源的线程在进一步申请其他资源时，如果申请不到，就主动释放掉已经占用的资源（指定时间内拿不到锁就把自己已经有的锁全部释放）。
