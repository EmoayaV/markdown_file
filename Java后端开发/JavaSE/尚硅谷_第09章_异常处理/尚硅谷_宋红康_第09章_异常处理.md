...
**`java.lang.Throwable` 类是Java程序执行过程中发生的异常事件对应的类的根父类。（是异常exception和错误error的根父类）**
...
* **运行时期异常**（即runtime异常、unchecked异常、非受检异常）：在代码编译阶段，编译器完全不做任何检查，无论该异常是否会发生，编译器都不给出任何提示。只有等代码运行起来并确实发生了xx异常，它才能被发现。通常，这类异常是**由程序员的代码编写不当引起的**，只要稍加判断，或者细心检查就可以避免。
...
![image-20230503164535552](D:\markdown file\截图\image-20230503164535552.png)

![image-20230503164504015](D:\markdown file\截图\image-20230503164504015.png)

![image-20230503165040194](D:\markdown file\截图\image-20230503165040194.png)

![image-20230531121945653](D:\markdown file\截图\image-20230531121945653.png)

![image-20230503171024403](D:\markdown file\截图\image-20230503171024403.png)

<img src="D:\markdown file\截图\image-20230504155659269.png" alt="image-20230504155659269" style="zoom:150%;" />

![image-20230504160104198](D:\markdown file\截图\image-20230504160104198.png)

- **trycatchfinally中即使在 try中用return的方式结束程序，还是在catch中存在运行时异常导致程序中断，都会在程序结束前执行finally的语句，简单来说无论什么情况finally的语句都会执行。**

- **try中声明的变量，出了try结构之后，就不可以进行调用了。**



![image-20230503222705291](D:\markdown file\截图\image-20230503222705291.png)

![image-20230503223658992](D:\markdown file\截图\image-20230503223658992.png)

![image-20230503224219565](D:\markdown file\截图\image-20230503224219565.png)

- **以上的道理适用于子类对于父类方法的重写，子类对于抽象父类的抽象方法的重写，实现类对于接口抽象方法的重写。以上要求只针对于编译时异常，不针对于运行时异常！！换句话说方法抛出运行时异常写了和没写一样。**
...
- thorw后的代码不能被执行
- try内部，如果出现异常信息则直接跳到相应catch语句，不执行异常后的代码
- 如果在main方法内部没有用trycatchfinally处理运行时异常，（对于编译时异常，必须用trycatchfinally处理异常，对于运行时异常，可以使用trycatchfinally处理异常也可以不用），那么出现异常时，异常代码后面的所有代码不执行。



- 抓抛模型：无论是throws自动抛出异常还是throw手动抛出异常，只要是运行时异常就不用去抓（trycatchfinally），只要是编译时异常要么抓（trycatchfinally），要么抛（throws）。



- 自定义异常的最大意义就是见名知意，知道发生了什么异常。
