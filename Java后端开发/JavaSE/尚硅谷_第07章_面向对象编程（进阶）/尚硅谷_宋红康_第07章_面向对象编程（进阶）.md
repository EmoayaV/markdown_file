...
![image-20230414150004305](D:\markdown file\截图\image-20230414150004305.png)

![image-20230414152441358](D:\markdown file\截图\image-20230414152441358.png)

![image-20230416123744879](D:\markdown file\截图\image-20230416123744879.png)

* this在方法：

  - 当形参与成员变量同名时，如果在方法内或构造器内需要使用成员变量，必须添加this来表明该变量是类的成员变量。（比如：this.name）

  - this可以作为当前类下的一个对象使用。如下
  ...
  - > ```java
    ...
    > 
    >         boy.marry(this); // this表示调用.marry()方法的对象
    >     }
    >     
    >     /**
    >      * 比较两个Girl对象的大小。
    ...
    >         if(this.age > girl.age){ // this.age表示调用.compare()方法的对象的属性
    >             return 1;            // girl.age表示放入.compare()方法的对象的属性
...
* this在构造器：
  ...
  - 当形参与成员变量同名时，如果在方法内或构造器内需要使用成员变量，必须添加this来表明该变量是类的成员变量。（比如：this.name）
  - this可以作为一个类中构造器相互调用的特殊格式。（比如：this()：调用本类的无参构造器，this(实参列表)：调用本类的有参构造器）
...
![image-20230415172704231](D:\markdown file\截图\image-20230415172704231.png)

![image-20230415172748779](D:\markdown file\截图\image-20230415172748779.png)

![image-20230416124210981](D:\markdown file\截图\image-20230416124210981.png)

* 属性的重写没有意义（属性是不能覆盖的，方法才可以覆盖），父类private权限的属性在子类中无法调用，父类public属性可以调用但是一般不会调用（可能？），如果在子类中出现和父类名字相同的属性，内存中会出现两个同名的ID，那么后面调用子类的属性，则会以子类的属性为准。
...
* 方法的重写注意：
  ...
  - 返回值类型是void，则子类重写的方法的返回值类型必须是void
  - 返回值类型是基本数据类型，则子类重写的方法的返回值类型必须与被重写的方法的返回值类型相同
  - 返回值类型是引用数据类型(比如类、字符串)，则子类重写的方法的返回值类型可以与被重写的方法的返回值类型相同 或 是被重写的方法的返回值类型的子类
  - 子类重写的方法抛出的异常类型可以与父类被重写的方法抛出的异常类型相同，或是父类被重写的方法抛出的异常类型的子类

  **简单起见，父类被重写的方法与子类重写的方法的权限修饰符、返回值、方法名、形参列表、异常类型都保持一致，只用改写方法体部分即可。**

**面试题：区分方法的重载(overload)与重写(override / overwrite)**

**重载：“两同一不同” 同一类内同一方法不同参数列表**
...
![image-20230416123726766](D:\markdown file\截图\image-20230416123726766.png)

![image-20230416133917094](D:\markdown file\截图\image-20230416133917094.png)

### 5.1 super的理解

在Java类中使用super来调用父类中的指定操作：

- **super可用于访问父类中定义的属性（super.）**
- **super可用于调用父类中定义的成员方法（super.）**
- **super可用于在子类构造器中调用父类的构造器（super(arg1, arg2)）**
...
![image-20230416133924988](D:\markdown file\截图\image-20230416133924988.png)
...
![image-20230416212736227](D:\markdown file\截图\image-20230416212736227.png)
...
![image-20230418141212306](D:\markdown file\截图\image-20230418141212306.png)

![image-20230515113753913](D:\markdown file\截图\image-20230515113753913.png)

**最重点的！！！！**

- **多态的使用前提：① 类的继承关系  ② 子类对父类方法的重写**
- **虚拟方法的调用：编译看左边，执行看右边**
- **虚方法适用于方法，不适用于属性：编译看左边，执行看左边，即多态性不适用于属性**
- **多态的弊端：一个引用类型变量如果声明为父类的类型，但实际引用的是子类对象，那么该变量就不能再访问子类中添加的属性和方法（无法使用未重写的方法，可以通过向下转型解决）。**
- **属性不存在多态性，只存在于方法！！！！**
...
**比如：**Person类有两个子类 man 和 woman，p是Person类的对象**
     **则 p instanceOf Person**
       **p instanceOf Object**
       **p instanceOf man**
       **p instanceOf man**
     都为true，只要和person有子父类关系（包括父类的父类，子类的子类）的都判断为true**
...
![image-20230418152114299](D:\markdown file\截图\image-20230418152114299.png)

![image-20230418152648828](D:\markdown file\截图\image-20230418152648828.png)

![image-20230418160534035](D:\markdown file\截图\image-20230418160534035.png)

- **Object类源码中equals()的作用与“==”相同：只能比较引用类型，比较是否指向同一个对象。**
- **String类的源码中对equals()方法进行了重写。**
- **当用equals()方法进行比较时，对类File、String、Date及包装类（Wrapper Class）来说，是比较类型及内容而不考虑引用的是否是同一个对象；在这些类中重写了Object类的equals()方法**
...
- **像String、File、Date或包装类等Object的子类，它们都重写了Object类的toString()，在调用toString()时，**
    **返回当前对象的实体内容。**
- **如果重写的toString方法中的变量是 class 或者 enum 的数据类型，那么返回的是一个该数据类型下的toString方法，注意也要重写此变量的toString方法。**
