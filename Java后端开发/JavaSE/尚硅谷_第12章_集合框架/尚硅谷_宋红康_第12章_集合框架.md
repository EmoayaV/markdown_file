...
- 在程序开发中，经常需要遍历集合中的所有元素。针对这种需求，JDK专门提供了一个接口`java.util.Iterator`。**`Iterator`接口也是Java集合**中的一员，但它与`Collection`、`Map`接口有所不同。
...
- ***迭代器的next*方法*返回*值*类型*是Object,所以要记得类型转换。**
...
![image-20230520121204391](D:\markdown file\截图\image-20230520121204391.png)

- P1根据属性得到哈希值，然后将其存储到内存中相应的位置，如果P1的属性改变，那么其哈希值也会改变，但是存储的位置不会改变，这时如果再删除P1，就会用更改后P1的属性计算哈希值，删除此哈希值位置下内存的数据，但是这个位置没有存储数据，所以就不能成功删除。
...
![image-20230520144210765](D:\markdown file\截图\image-20230520144210765.png)

- 遍历key和遍历value的位置是对应的
